...
def _non_overlapping_intervals(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep the finest non-overlapping age partition available.

    Eurostat may return both single ages and aggregate classes. Sorting by interval
    width ensures that single ages are selected first and overlapping aggregate
    classes are discarded without distributing values across ages.
    """
    if frame.empty:
        return frame
    grouped = (
    ...
    grouped = grouped.sort_values(["width", "age_low", "age_high"])
    selected: list[int] = []
    intervals: list[tuple[int, int]] = []
    for row in grouped.itertuples():
        low, high = int(row.age_low), int(row.age_high)
        overlaps = any(not (high < chosen_low or low > chosen_high) for chosen_low, chosen_high in intervals)
        if overlaps:
            continue
...
def atomic_population_rows(population: pd.DataFrame) -> pd.DataFrame:
    ...
    keys = [column for column in _group_keys(population) if column not in {"source"}]
    ...
    for key_values, group in population.groupby(keys, dropna=False):
        ...
        for sex, sex_group in group.groupby("sex", dropna=False):
            source = sex_group["source"].iloc[0] if "source" in sex_group else None
    ...
    columns = [column for column in population.columns if column in pieces[0].columns]
    return pd.concat(pieces, ignore_index=True, sort=False)[columns]


def _band_sum(group: pd.DataFrame, lower: int, upper: int) -> float:
    intersects = group["age_high"].ge(lower) & group["age_low"].le(upper)
    partial = intersects & ~(group["age_low"].ge(lower) & group["age_high"].le(upper))
    if partial.any():
        return np.nan
...
def compute_age_structure(population: pd.DataFrame) -> pd.DataFrame:
    ...
    for key, group in clean.groupby(keys, dropna=False):
        ...
        for name, (lower, upper) in AGE_BANDS.items():
            row[name] = _band_sum(total_by_age, lower, upper)
            row[f"share_{name.removeprefix('pop_')}"] = (
                100 * row[name] / total if total and pd.notna(row[name]) else np.nan
            )
        ...
        row["women_15_49"] = _band_sum(group[group["sex"].eq("F")], 15, 49) if female else np.nan
        ...
        row["support_ratio_15_64_per_65_plus"] = working / old if old and pd.notna(working) else np.nan
        ...
        row["age_resolution"] = "single_year" if not widths.empty and widths.max() == 0 else "grouped"
        rows.append(row)
...
def _select_plot_source(subset: pd.DataFrame, iso3: str) -> pd.DataFrame:
    ...
    for source in priorities:
        ...
        if not selected.empty:
            return selected
    ranks = subset["source"].map(SOURCE_PRIORITY).fillna(99)
    return subset[ranks.eq(ranks.min())]
